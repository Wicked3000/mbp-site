...
const FODE_INTAKE_LIST = [
  'FODE Intake 2026'
];

export default function FodeStudentsPage() {
  ...
  const [formData, setFormData] = useState({
    candidate_name: '',
    primary_school: '',
    destination_school: 'FODE Intake 2026',
    gender: 'M',
    status: 'Selected'
  });
  ...
  const loadStudents = async (school = selectedSchool) => {
    setLoading(true);
    try {
      let url = `/api/fode-students?school=${encodeURIComponent(school)}`;
      ...
      console.error('Failed to load FODE students:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchInit = async () => {
      setLoading(true);
      try {
        let url = `/api/fode-students?school=${encodeURIComponent(selectedSchool)}`;
        const res = await fetch(url);
        if (!ignore && res.ok) {
          const data = await res.json();
          setStudents(data);
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/fode-students', {
        method: 'POST',
      ...
      if (res.ok && data.success) {
        setMessage('FODE candidate added successfully!');
        setFormData({
          candidate_name: '',
          primary_school: '',
          destination_school: 'FODE Intake 2026',
        ...
        setMessage('Failed to add FODE candidate: ' + (data.error || 'Unknown error'));
      }
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Are you sure you want to remove this candidate from the FODE selection list?')) return;

    try {
      const res = await fetch(`/api/fode-students?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
        loadStudents();
      }
    } catch (err) {
      console.error('Error deleting FODE candidate:', err);
    }
  };

  const handleExportTextFile = () => {
    const schoolName = selectedSchool || 'All_FODE_Intake';

    let fileContent = `MILNE BAY ADMINISTRATION\n`;
    fileContent += `OFFICIAL FODE INTAKE LIST FOR 2026\n`;
    fileContent += `===========================================================\n\n`;
    fileContent += `Intake: ${selectedSchool || 'All Milne Bay Province FODE Intake'}\n`;
    fileContent += `Total Candidates: ${filteredStudents.length}\n\n`;
    fileContent += `No. | Candidate Name             | Primary School             | Gender | Intake                | Status\n`;
    fileContent += `---------------------------------------------------------------------------------------------------------\n`;

    filteredStudents.forEach((s, idx) => {
      const name = (s.candidate_name || s.name || '').padEnd(26);
      const prev = (s.primary_school || s.prev || '').padEnd(26);
      const gender = String(s.gender || 'M').padEnd(6);
      const dest = (s.destination_school || '').padEnd(21);
      const stat = s.status || 'Selected';
      fileContent += `${String(idx + 1).padEnd(3)} | ${name} | ${prev} | ${gender} | ${dest} | ${stat}\n`;
    });
    ...
    link.href = url;
    link.download = `FODE_Selection_List_${schoolName.replace(/\s+/g, '_')}.txt`;
    ...
    reader.onload = async (e) => {
      ...
      const studentsData = filteredRows.map(row => {
        ...
        return {
          ...
          destination_school: columns[2] ? columns[2].trim() : 'FODE Intake 2026',
          status: columns[3] ? columns[3].trim() : 'Selected',
          gender: columns[4] ? columns[4].trim() : 'M'
        };
      });
      ...
      setMessage(`Successfully added ${result.addedStudents.length} FODE candidates. ${
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 pb-6 border-b border-[#1565C0]/40">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-white tracking-tight font-display flex items-center space-x-3">
              ...
              <span>FODE Selection List</span>
            </h1>
            <p className="text-slate-400 text-sm mt-1">
              Flexible Open & Distance Education intake records management for Milne Bay Province
            </p>
          </div>
            ...
            <div className="glass-panel max-w-lg w-full p-6 border border-[#1565C0]/40 shadow-2xl relative">
              ...
              <form onSubmit={handleBulkUpload} className="space-y-4">
                <p className="text-sm text-slate-500 mb-4">
                  Upload a CSV file to add multiple FODE candidates to the selection list.
                  <br className="hidden md:block" />
                  <span className="text-xs text-slate-500">
                    Format: candidate_name,primary_school,destination_school,status,gender
                  </span>
                </p>
                <div>
                  <label
                ...
                <div>
                  <p className="text-sm text-slate-500 mb-2">
                    Showing {students.length} candidate(s) in current view. Uploaded
                    candidates will be added to the existing list.
                  </p>
                </div>
                <div>
                  <button
                  ...
                  <button
                    ...
                    className="btn-gold w-full text-xs font-medium py-2"
                  >
                    {bulkUploading ? 'Uploading...' : 'Upload Candidates'}
                  </button>
                </div>
              </form>
            </div>
        ...
        <div className="glass-panel p-4 mb-6 border border-[#1565C0]/40 flex flex-col md:flex-row md:items-center gap-4">
          ...
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-2">
              ...
              >
                <option value="">All FODE Intake</option>
                {FODE_INTAKE_LIST.map((sch) => (
            ...
            <table className="w-full text-left text-sm text-slate-300">
              <thead className="bg-[#0D47A1]/80 text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-[#1565C0]/40">
                <tr>
                  ...
                  <th className="px-5 py-3.5">Primary School</th>
                ...
                {loading ? (
                  <tr>
                    <td colSpan="7" className="px-5 py-8 text-center text-slate-500">
                      Loading FODE selection list...
                    </td>
                  </tr>
                ) : filteredStudents.length === 0 ? (
                  <tr>
                    <td colSpan="7" className="px-5 py-8 text-center text-slate-500">
                      No FODE candidates found matching your filters.
                    </td>
                  </tr>
                ) : (
        ...
        <div className="fixed inset-0 z-50 bg-[#0a192f]/85 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="glass-panel max-w-lg w-full p-6 border border-[#1565C0]/40 shadow-2xl relative">
            <div className="flex items-center justify-between mb-5 pb-3 border-b border-[#1565C0]/40">
              <h3 className="font-bold text-lg text-white">Add Candidate to FODE Selection List</h3>
              <button
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Lelohoa Primary School"
                  className="w-full px-3 py-2 bg-[#0a192f] border border-[#1565C0]/50 rounded-lg text-sm text-white focus:outline-none focus:border-amber-400"
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Intake (Destination) *</label>
                <select
