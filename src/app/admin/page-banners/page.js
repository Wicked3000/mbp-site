...
const PAGE_META = {
  ...
  basic: { label: 'Basic Education', path: '/basic' },
  post: { label: 'Post Primary Education', path: '/post' },
  vet: { label: 'Vocational Education (VET)', path: '/vet' },
  fode: { label: 'Flexible Open & Distance Education', path: '/fode' }
};
...
export default function PageBannersPage() {
  ...
  const handleSave = async () => {
    if (!editing || !form.image_url.trim()) return;
    setSaving(true);
    try {
      const res = await fetch(`/api/page-banners?id=${editing.id}`, {
        method: 'PATCH',
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 pb-6 border-b border-[#1565C0]/40">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-white tracking-tight font-display flex items-center space-x-3">
              ...
              <span>Page Banner Manager</span>
            </h1>
            <p className="text-slate-400 text-sm mt-1">
              Customize the banner image, title and subtitle shown on the About, Basic, Post Primary, VET and FODE pages
            </p>
          </div>
        ...
        <div className="glass-panel border border-[#1565C0]/40 overflow-hidden rounded-xl">
            ...
            <table className="w-full text-left text-sm text-slate-300">
              <thead className="bg-[#0D47A1]/80 text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-[#1565C0]/40">
                <tr>
                  ...
                  <th className="px-5 py-3.5">Banner Preview</th>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {loading ? (
                  ...
                  banners.map((n) => {
                    const meta = PAGE_META[n.page_key] || { label: n.page_key, path: `/${n.page_key}` };
                    return (
                      <tr key={n.id} className="hover:bg-[#1565C0]/30 transition-colors">
                        <td className="px-5 py-4">
                          <span className="font-semibold text-white">{meta.label}</span>
                          <span className="block text-[11px] text-slate-500 font-mono mt-0.5">/{n.page_key}</span>
                        </td>
                        <td className="px-5 py-4">
                          {n.image_url ? (
                            <img src={n.image_url} alt={meta.label} className="w-40 h-16 object-cover rounded border border-[#1565C0]/40" />
                          ) : (
                            <div className="w-40 h-16 rounded border border-dashed border-[#1565C0]/40 flex items-center justify-center text-slate-500">
                              <Image className="w-4 h-4" />
                            </div>
                          )}
                        </td>
                        <td className="px-5 py-4 font-medium text-white max-w-xs truncate">{n.title}</td>
                        <td className="px-5 py-4 text-slate-300 max-w-xs truncate">{n.subtitle}</td>
                        <td className="px-5 py-4 text-right space-x-2 whitespace-nowrap">
                          <a
                            href={meta.path}
                            target="_blank"
                            rel="noreferrer"
                            className="px-2.5 py-1.5 bg-emerald-500/15 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/30 rounded text-xs font-medium inline-flex items-center space-x-1"
                          >
          ...
          <div className="p-4 bg-[#0D47A1]/50 border-t border-[#1565C0]/40 flex items-center justify-between text-xs text-slate-400">
            <span>Total {banners.length} page banner(s)</span>
            <span>Changes appear on the live pages immediately</span>
          </div>
        </div>
      </main>
          ...
          <div className="glass-panel max-w-lg w-full p-6 border border-[#1565C0]/40 shadow-2xl relative">
            ...
            <div className="mb-5">
              <span className="text-xs font-semibold text-amber-400 uppercase tracking-wider">
                {PAGE_META[editing.page_key]?.label || editing.page_key}
              </span>
              <span className="text-xs text-slate-500 font-mono ml-2">/{editing.page_key}</span>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex items-center space-x-3 mb-3">
                  {form.image_url ? (
                    <img src={form.image_url} alt="Banner preview" className="w-40 h-24 object-cover rounded border border-[#1565C0]/40" />
                  ) : (
                    <div className="w-40 h-24 rounded border border-dashed border-[#1565C0]/40 flex items-center justify-center text-slate-500">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-[#0a192f] border border-[#1565C0]/50 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:border-amber-400"
                  placeholder="e.g. About Milne Bay Province"
                />
              </div>
