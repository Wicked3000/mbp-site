...
| MySQL (Railway) | Persistent data (news, notices, policies, students, contacts, banners, page banners, welcome message) |
...
Public GET endpoints (no auth): `students`, `notices`, `news`, `latest-news`, `banners`, `page-banners`, `welcome`, `policies`, `contacts` (read-only). All mutations require a session.
...
| `/api/welcome` | Homepage welcome message (kicker, heading, body, image) |
